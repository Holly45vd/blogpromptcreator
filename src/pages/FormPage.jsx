...
const concernOptions = [
  ...
  "주차 정보 확인 필요",
  "피크 시간대는 사람이 많을 수 있음",
  "처음 가면 입구/위치가 살짝 헷갈릴 수 있음",
  "원하는 시간대는 미리 문의하는 게 좋음",
  "사진과 실제 분위기가 조금 다르게 느껴질 수 있음",
  "후기가 많지 않아 직접 확인이 필요했음",
  "사람에 따라 만족 포인트가 다를 수 있음",
];
...
const requiredFieldsByStep = {
  ...
  1: {
    primaryKeyword: "핵심 검색 키워드를 입력해주세요.",
  },
  2: {
...
function FormPage() {
  ...
  const buildPromptData = () => {
    ...
    const realExperience = combineText([
      ...
      formData.experienceNumbersHint &&
        `기억나는 숫자 정보: ${formData.experienceNumbersHint}`,
    ]);
    ...
    const badPoint = combineText([
      joinArray(formData.selectedConcerns) &&
        `방문 전 확인하면 좋을 점: ${joinArray(formData.selectedConcerns)}`,
      formData.badPointDetail && `애매했던 점/참고할 점 상세: ${formData.badPointDetail}`,
    ...
    const writingStyleGuide = combineText([
      ...
      "너무 완벽하게 정리된 문장만 쓰지 말고, 실제 경험자의 말처럼 문장 길이에 변화를 주세요.",
      "장점은 크게 칭찬하기보다 실제로 좋았던 상황을 보여주는 방식으로 써주세요.",
      "아쉬운 점은 부정적으로 몰아가지 말고, 방문 전 확인하면 좋은 참고점처럼 써주세요.",
    ...
    return {
      ...
      tone: formData.tone,
      writingStyleGuide,
      summaryHint: formData.summaryHint,
      recommendTargetHint: formData.recommendTargetHint,
      avoidTargetHint: formData.avoidTargetHint,
      experienceNumbersHint: formData.experienceNumbersHint,
  ...
  const renderStepContent = () => {
    ...
    if (currentStep === 1) {
      return (
        <SectionCard
          title="2단계. 검색 키워드"
          description="핵심 검색어와 보조 키워드를 정리합니다. 지역 키워드는 보조 키워드에 함께 넣으면 됩니다."
        >
          <InputField
            label="핵심 검색 키워드"
            name="primaryKeyword"
            ...
            description="가장 노출시키고 싶은 핵심 검색어입니다. 지역명까지 포함해도 괜찮아요."
            example="동래 헬스장 / 치앙마이 숙소 추천 / 용산왁싱 / 마운자로 식단"
          />

          <InputField
            label="보조 검색 키워드"
            name="subKeywords"
            value={formData.subKeywords}
            onChange={handleChange}
            placeholder="예: 부산 동래, 동래역, 가격, 주차, 예약"
            description="지역, 역명, 상황, 가격/예약/주차처럼 함께 노출되면 좋은 키워드를 입력해주세요. 필수는 아니에요."
          />
    ...
    if (currentStep === 2) {
      return (
        ...
        >
          ...
          <TextAreaField
            label="기억나는 숫자 정보"
            name="experienceNumbersHint"
            value={formData.experienceNumbersHint}
            onChange={handleChange}
            placeholder="예: 대기 5분 정도, 사진 8장, 상담 20분, 2층, 역에서 도보 7분"
            description="가격, 시간, 거리, 횟수처럼 기억나는 숫자가 있으면 적어주세요. 비워두면 AI가 입력값 안에서만 자연스럽게 정리합니다."
            rows={3}
          />
        </SectionCard>
      );
    ...
    if (currentStep === 3) {
      return (
        ...
        >
          ...
          <ChipGroup
            label="방문 전 확인하면 좋을 점"
            name="selectedConcerns"
            selectedValues={formData.selectedConcerns}
            onChange={handleChipChange}
            options={concernOptions}
            description="부정적인 단점보다, 독자가 미리 확인하면 좋을 참고점을 선택해주세요."
          />
          ...
          <TextAreaField
            ...
            onChange={handleChange}
            placeholder="예: 주차나 예약 가능 여부는 방문 전에 한 번 확인하는 게 편할 것 같다."
            description="필수는 아닙니다. 사람에 따라 다르게 느낄 만한 부분이나 방문 전 확인할 점이 있으면 적어주세요."
            rows={4}
          />
    ...
    return (
      ...
      >
        ...
        <TextAreaField
          label="내가 생각한 한 줄 느낌"
          name="summaryHint"
          value={formData.summaryHint}
          onChange={handleChange}
          placeholder="예: 엄청 특별하다기보다는 처음 가는 사람이 필요한 정보를 확인하기 편했다."
          description="필수는 아닙니다. 비워두면 AI가 전체 입력값을 보고 자연스럽게 결론을 만듭니다."
          rows={3}
        />

        <TextAreaField
          label="추천하면 좋을 사람"
          name="recommendTargetHint"
          value={formData.recommendTargetHint}
          onChange={handleChange}
          placeholder="예: 처음 방문 전 가격, 위치, 예약 정보를 한 번에 보고 싶은 사람"
          description="직접 쓰고 싶을 때만 입력해주세요. 비워두면 AI가 타깃 독자와 경험 내용을 보고 판단합니다."
          rows={3}
        />

        <TextAreaField
          label="확인하고 가면 좋을 사람"
          name="avoidTargetHint"
          value={formData.avoidTargetHint}
          onChange={handleChange}
          placeholder="예: 주차가 꼭 필요한 사람은 방문 전 확인하면 좋을 것 같다."
          description="비추천이 아니라 확인 필요 대상입니다. 필수는 아니에요."
          rows={3}
        />
